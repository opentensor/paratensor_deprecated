...
impl<T: Config> Pallet<T> {
    ...
    pub fn update_difficulty() { 
        ...
        // Difficulty should never exceed this value.
        let max_difficulty: u64 = Self::get_maximum_difficulty(); 

        // Difficulty should never be below this value
        let min_difficulty: u64 = Self::get_minimum_difficulty();

        // Number of blocks average registrations are taken over.
        let adjustment_interval: u64 = Self::get_adjustment_interval();

        // Current difficulty
        let current_difficulty: u64 = Self::get_difficulty_as_u64();

        // Target number of registrations on average over interval.
        let target_registrations_per_interval: I65F63 = I65F63::from_num( Self::get_target_registrations_per_interval() );
        ...
        // --- Check if we have reached our adjustment interval.
        if current_block - last_adjustment >= adjustment_interval {
            ...
            // --- Update the last adjustment to current block and zero the registrations since the last difficulty.
